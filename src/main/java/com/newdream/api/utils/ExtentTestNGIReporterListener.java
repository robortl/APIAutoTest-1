    ...
    private void buildTestNodes(ExtentTest extenttest, IResultMap tests, Status status) {
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                //如果有参数，则使用参数中的CaseName来作为报告的name
                String[] parameters_String = parameters[0].toString().split(",");
                String nameTemp = "";
                for (int i = 0; i<parameters_String.length;i++){
                    String value = parameters_String[i];
                    if (value.contains("CaseName")){
                        nameTemp = value.split("=")[1];
                        name = nameTemp;
                    }
                }
